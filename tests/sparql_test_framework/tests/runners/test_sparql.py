...
class TestSparqlTestRunner(SparqlTestFrameworkTestBase):
    ...
    def test_use_rasqal_compare_flag_exists(self):
        ...
        args = parser.parse_args(["--manifest-file", "test.ttl", "--use-rasqal-compare"])
...
class TestRasqalCompareIntegration(SparqlTestFrameworkTestBase):
    ...
    @patch('sparql_test_framework.runners.sparql.compare_with_rasqal_compare')
    @patch('sparql_test_framework.runners.sparql.run_command')
    @patch('sparql_test_framework.runners.sparql.DIFF_OUT')
    def test_compare_actual_vs_expected_with_rasqal_compare(self, mock_diff_out, mock_run_command, mock_rasqal_compare):
        ...
        # Mock rasqal-compare result
        mock_rasqal_compare.return_value = 1
        ...
        with patch('builtins.open', create=True):
            ...
            with patch('pathlib.Path.exists', return_value=True):
                with patch('pathlib.Path.stat') as mock_stat:
                    ...
                    with patch('sparql_test_framework.runners.sparql.read_query_results_file') as mock_read:
                        ...
                        # Verify rasqal-compare was called
                        mock_rasqal_compare.assert_called_once()

    @patch('sparql_test_framework.runners.sparql.run_command')
    @patch('sparql_test_framework.runners.sparql.DIFF_OUT')
    def test_compare_actual_vs_expected_without_rasqal_compare(self, mock_diff_out, mock_run_command):
    ...
    def test_run_single_test_passes_use_rasqal_compare(self, mock_execute_roqet, mock_compare):
