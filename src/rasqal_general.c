 ...
 *
 * Allows customizing of the level of warnings (possible errors) that
 * are generated by Rasqal.  Level 0 generates no warnings. Level 50
 * will generate, style and serious warnings.  Level is 100 will show
 * all warnings including strict style warnings that will never
 * affect query execution results..
 *
 * Return value: pointer to timeval or NULL on failure
 */
int
