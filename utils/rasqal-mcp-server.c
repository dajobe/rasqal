...
static int
create_jsonrpc_response(const char* id, const char* result, const char* error,
                        char* buffer, size_t* len)
{
  ...
  if(result) {
    raptor_iostream_string_write((const unsigned char*)",\"result\":", iostr);
    raptor_iostream_string_write((const unsigned char*)result, iostr);
  } else if(error) {
    raptor_iostream_counted_string_write(",\"error\":", 9, iostr);
    raptor_iostream_string_write((const unsigned char*)error, iostr);
  }
  ...
  if(output_string) {
    ...
    log_message(RAPTOR_LOG_LEVEL_DEBUG, "create_jsonrpc_response - id: '%s', result: '%s', error: '%s'",
                id ? id : "null", result ? result : "null", error ? error : "null");
    log_message(RAPTOR_LOG_LEVEL_DEBUG, "Response buffer content: '%s'", buffer);
    log_message(RAPTOR_LOG_LEVEL_DEBUG, "Response buffer length: %zu", *len);
